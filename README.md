...
- ラン：実GPSを約5秒間隔で要求。一時停止／再開／終了、GPS軌跡、1kmごとの自動ラップ。専用の一時停止ボタンはラップ一覧に変更。
...
設定は一度だけです。一般利用者を個別追加せずに利用させるには、Google OAuthのAudienceと公開状態も確認してください。Vercelの認証画面が先に出る場合はアプリのGoogleログインとは別のDeployment Protectionです。本番の保護設定を確認します。管理用Vercel/Supabaseアカウントがアプリ認証で統合されるわけではありません。
...
Apple Watchの歩数・歩行＋走行距離を、iPhoneのAppleヘルスケアからショートカット経由でHTTP POSTして同期する構成です。Duffyは市販の歩数表示アプリで、コードを入力してこのアプリへ直接送信する機能は確認できていません。マイページの「Apple Watch連携（Duffy）」から連携コードを発行し、
iPhoneのショートカットにコードとエンドポイントを設定します。画面内の設定手順は `public/health-sync.html`。iPhone実機での同期は未検証です。技術仕様は[docs/apple-watch-sync.md](docs/apple-watch-sync.md)を参照。
...
- GPSの連続監視を5秒間隔の単発要求に置換。一時停止中は取得停止、遅延応答は破棄。実際のハードウェア稼働・電池消費は端末依存。
- 左ボタンはラップ一覧、中央は開始／一時停止／再開、右は終了。速度を直近1kmラップに変更。
- 添付に合わせたPR枠3箇所。結果に海鮮写真、ホームに店舗アイコン、全幅CTA。広告主・リンクはサンプル。
- Apple Health同期SQLを接続先Supabaseに適用し、送信・同日上書き・日本時間の日付・トークン解除をロールバックする試験で確認。健康データもアプリデータ削除の対象に追加。
- iPhoneショートカット経由の設定手順を追加。Duffy直接接続、HealthKitネイティブ同期、iPhone実機確認は未完了。PDL本体への保存は既存と同様未接続。
- Googleログインは既存実装を維持。Vercel接続先に対象チームがなく、Deployment Protectionの確認・変更は未実施。SupabaseのGoogleプロバイダー公開設定もこの環境では確認できていない。
- ブラウザがローカル確認URLをERR_BLOCKED_BY_CLIENTで拒否したため、画面の目視検証は未実施。
