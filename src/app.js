...
const tracker=new RunTracker();let profile={nickname:'Runner',country:'jp',consents:{},unit:'km'},runs=[],latest=null,watch=null,timer=null,wake=null,screen='login',saving=false,saveError=false,ready=false,team=null,challengeList=[],challengeEntries=[],rankRequest=0,previousFocus=null;
...
function field(label,id,value='',type='text'){return `<label for="${id}">${esc(label)}</label><input id="${id}" type="${type}" value="${esc(value)}" ${type==='password'?'minlength="8" autocomplete="current-password"':type==='email'?'autocomplete="email"':''} required maxlength="${type==='text'?40:254}">`;}
async function openAuth(){if(!data.supabase){modal('クラウド接続の準備中',note('メールログインはまだ設定されていません。「この端末だけに保存して使う」から計測を始められます。'));return;}modal('メールでログイン',`<form id="authForm">${field('メールアドレス','email','','email')}${field('パスワード（8文字以上）','password','','password')}<p id="authError" class="form-error" role="alert"></p><button class="primary-btn" type="submit">ログイン</button><button class="secondary-btn" type="submit" value="signup" style="margin-top:8px">新規登録</button></form>`);$('#authForm').onsubmit=async e=>{e.preventDefault();const signup=e.submitter.value==='signup';$$('button',e.target).forEach(b=>b.disabled=true);try{const ok=await data.signIn($('#email').value,$('#password').value,signup);if(ok){await loadApp();close();navigate('home');}else text('#authError','確認メールを送信しました。メール内のリンクを開いてからログインしてください。');}catch(err){text('#authError',err.message)}finally{$$('button',e.target).forEach(b=>b.disabled=false);}};}
async function loadApp(){const result=await data.load();profile=result.profile;runs=result.runs;ready=true;latest=runs[0]||null;renderSummary();}
...
const informational={
 ...
 'consent':['Jasmy PDL連携は準備中','PDLの認証先とAPI接続はまだ設定されていません。PDLへデータは送信されません。メールログイン、または端末保存をご利用ください。'],
