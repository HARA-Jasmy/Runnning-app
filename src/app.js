...
const data=await (demo?import('./demo-data.js'):import('./data.js'));
...
function navigate(name){if(saveError&&name!=='result'){toast('未保存の走行記録を再保存してください。');return;}if(!ready&&name!=='login')return;screen=name;$$('.screen').forEach(el=>{const active=el.id===`screen-${name}`;el.classList.toggle('active',active);el.inert=!active;});$('#bottomNav').classList.toggle('visible',name!=='login');const key=$(`#screen-${name}`)?.dataset.nav;$$('[data-nav-go]').forEach(b=>b.classList.toggle('active',b.dataset.navGo===key));if(name==='ranking')safe(renderRanking)();if(name==='team')safe(renderTeam)();if(name==='challenge')safe(renderChallenge)();if(name==='run')updateRun();}
...
function renderSummary(){$$('.avatar').forEach(img=>img.src=profile.avatar_data||'/assets/avatar-default.svg');if($('#rankingConsent')){$('#rankingConsent').classList.toggle('on',!!profile.consents.ranking);$('#rankingConsent').setAttribute('aria-checked',!!profile.consents.ranking);}const s=summarize(runs);const kmFactor=factor();text('.greeting','今日も、自分のペースで');text('.greeting-name',`${profile.nickname}さん ☀️`);text('#profileName',profile.nickname);const country=({jp:'Japan',us:'United States',sg:'Singapore',kr:'Korea'})[profile.country]||profile.country;$('.profile-copy > span').innerHTML=`${profile.country==='jp'?'<img class="country-flag" src="/assets/flag-jp.svg" alt="日本の国旗">':`<span aria-hidden="true">${({us:'🇺🇸',sg:'🇸🇬',kr:'🇰🇷'})[profile.country]||'🌐'}</span>`} ${esc(country)}`;text('.home-date',new Date().toLocaleDateString(locale(),{month:'long',day:'numeric',weekday:'short'}));text('#storageStatus',demo?'デモ表示 · 変更はこの画面内のみ':data.isGuest()?'端末保存モード · このブラウザに保存します':'クラウド保存 · PDLは未連携');
 ...
 text('#totalDistance',(s.totalDistance*kmFactor).toFixed(1));text('#screen-mypage .profile-stat span',`総距離 ${unit()}`);text('#totalRuns',s.totalRuns);text('#totalDays',s.totalDays);text('#unitValue',`${unit()} ›`);text('#homeContribution',team?`${Number(team.my_recorded_km||0).toFixed(2)} km`:'—');text('#homeEntries','開催情報を確認');text('[data-go="ranking"] .value-lg',demo?'#3':'—');text('.rank-change','認定ランを集計');text('[data-go="team"] .value-lg',team?`${Number(team.recorded_distance_km??team.distance_km??0).toFixed(2)} km`:team===undefined?'取得できません':'未参加'); $('[data-go="team"] .small').innerHTML=`今月の記録距離（未認定を含む）<br>あなたの記録 <b id="homeContribution">${team?Number(team.my_recorded_km||0).toFixed(2)+' km':'—'}</b>`;text('[data-go="team"] .team-mini-name',team?.name||'チーム');text('.challenge-mini-copy p','走って北海道グルメを当てよう！');
...
function startRun(){if(saveError)throw Error('前の記録を保存してから開始してください。');navigate('run');}
...
async function sheet(kind){if(saveError&&kind==='history'){toast('未保存の走行記録を再保存してください。');return;}if(informational[kind]){modal(...[informational[kind][0],note(informational[kind][1])]);return;}
 ...
 const preview=$('#avatarPreview'),save=$('#saveProfileButton');preview.src=avatar||'/assets/avatar-default.svg';$('#country').value=profile.country;
 $('#removeAvatar').onclick=()=>{selection++;processing=false;save.disabled=false;avatar=null;preview.src='/assets/avatar-default.svg';$('#avatarFile').value='';text('#profileError','');};
...
async function invite(){if(demo){toast('デモのチームです。本番でチームを作成すると招待できます。');return;}if(!team)return;const u=new URL(location.origin);u.searchParams.set('team',team.name);try{await navigator.clipboard.writeText(u.href);toast('招待リンクをコピーしました。')}catch{modal('招待リンク',`<input readonly aria-label="招待リンク" value="${esc(u.href)}">`);}}
...
safe(async()=>{navigate('login');if(await data.restore()){await loadApp();navigate(demo&&['team','ranking','run','mypage'].includes(new URLSearchParams(location.search).get('screen'))?new URLSearchParams(location.search).get('screen'):'home');if(data.recovering)openPasswordSetup();}if(ready){$('#rankingConsent').classList.toggle('on',!!profile.consents.ranking);$('#rankingConsent').setAttribute('aria-checked',!!profile.consents.ranking);}updateRun();})();

$('#retryGPS').onclick=$('#locateGPS').onclick=safe(()=>{if(!tracker.active){toast('中央のスタートボタンを押すと位置情報を取得します。');return;}if(tracker.paused){toast('再開するとGPSを取得します。');return;}watchGPS();});
...
if(demo){document.body.classList.add('demo-mode');const banner=document.createElement('div');banner.className='demo-banner';banner.innerHTML='デモ · 架空のメンバー・記録 <a href="/">本番へ戻る</a>';$('.prototype-shell').prepend(banner);}
