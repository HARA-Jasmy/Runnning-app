...
function renderSummary(){$$('.avatar').forEach(img=>img.src=profile.avatar_data||'/assets/avatar-default.svg');if($('#rankingConsent')){$('#rankingConsent').classList.toggle('on',!!profile.consents.ranking);$('#rankingConsent').setAttribute('aria-checked',!!profile.consents.ranking);}const s=summarize(runs);const kmFactor=factor();text('.greeting','今日も、自分のペースで');text('.greeting-name',`${profile.nickname}さん ☀️`);text('#profileName',profile.nickname);const country=({jp:'Japan',us:'United States',sg:'Singapore',kr:'Korea'})[profile.country]||profile.country;$('.profile-copy > span').innerHTML=`${profile.country==='jp'?'<img class="country-flag" src="/assets/flag-jp.svg" alt="日本の国旗">':`<span aria-hidden="true">${({us:'🇺🇸',sg:'🇸🇬',kr:'🇰🇷'})[profile.country]||'🌐'}</span>`} ${esc(country)}`;text('.home-date',new Date().toLocaleDateString(locale(),{month:'long',day:'numeric',weekday:'short'}));text('#storageStatus',demo?'デモ表示 · 変更はこの画面内のみ':data.isGuest()?'端末保存モード · このブラウザに保存します':'クラウド保存 · PDLは未連携');
 ...
 text('#totalDistance',(s.totalDistance*kmFactor).toFixed(1));text('.profile-stat span',`総距離 ${unit()}`);text('#totalRuns',s.totalRuns);text('#totalDays',s.totalDays);text('#unitValue',`${unit()} ›`);text('#homeContribution',team?`${Number(team.my_recorded_km||0).toFixed(2)} km`:'—');text('#homeEntries','開催情報を確認');text('[data-go="ranking"] .value-lg',demo?'#3':'—');text('.rank-change','認定ランを集計');text('[data-go="team"] .value-lg',team?`${Number(team.recorded_distance_km??team.distance_km??0).toFixed(2)} km`:team===undefined?'取得できません':'未参加'); $('[data-go="team"] .small').innerHTML=`今月の記録距離（未認定を含む）<br>あなたの記録 <b id="homeContribution">${team?Number(team.my_recorded_km||0).toFixed(2)+' km':'—'}</b>`;text('[data-go="team"] .team-mini-name',team?.name||'チーム');text('.challenge-mini-copy p','走って北海道グルメを当てよう！');
